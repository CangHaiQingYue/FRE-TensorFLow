...
def spare_to_dense(h):
    return tf.sparse_tensor_to_dense(h, default_value=0)

def _parse_function(serialized_example):
    feature = {'image': tf.FixedLenFeature((), tf.string, default_value=''),
         'label': tf.FixedLenFeature((), tf.string, default_value=''),
         # 'h': tf.VarLenFeature(tf.int64),
         # 'w': tf.VarLenFeature(tf.int64),
         # 'c': tf.VarLenFeature(tf.int64)
    ...
    # h = tf.cast(spare_to_dense(parse_feature['h']), tf.int64)
    # w = tf.cast(spare_to_dense(parse_feature['w']), tf.int64)
    # c = tf.cast(spare_to_dense(parse_feature['c']), tf.int64)
    
    h = tf.cast(parse_feature['h'], tf.int64)
    w = tf.cast(parse_feature['w'], tf.int64)
    c = tf.cast(parse_feature['c'], tf.int64)

    # image_shape = tf.stack([h, w, c])
    image = image.set_shape(tf.stack([h, w, c]))
    image 
    ...
    # if label.get_shape().as_list()[-1] > 1:
        # print('Needed to be sliced')
